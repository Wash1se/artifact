...
export const PortfolioSwiper: React.FC<PortfolioSwiperProps> = ({
  images,
  altPrefix,
  ...
  const swapTimeoutIds = useRef<number[]>([]);
  ...
  const handleEnd = useCallback(() => {
    ...
    if (card) {
      ...
      if (Math.abs(deltaX) > threshold) {
        ...
        const orderTimeoutId = window.setTimeout(() => {
          setCardOrder(prev => {
        ...
        swapTimeoutIds.current.push(rotateTimeoutId, orderTimeoutId);
      } else {
        applySwipeStyles(0);
      }
    }
  ...
  useEffect(() => {
    ...
    observer.observe(cardStackElement);
    ...
    return () => {
      ...
      swapTimeoutIds.current.forEach(window.clearTimeout);
      swapTimeoutIds.current = [];
    };
  ...
  useEffect(() => {
    ...
    const nextVisible = cardOrder.slice(0, 1);

    setLoadedIndices((current) => {
      const merged = new Set(current);
      nextVisible.forEach((index) => merged.add(index));
      return Array.from(merged).sort((first, second) => first - second);
    });
  }, [cardOrder, shouldLoadImages]);
  ...
  useEffect(() => {
    ...
    const preloadNext = () => {
      setLoadedIndices((current) => {
        const merged = new Set(current);
        cardOrder.slice(0, 2).forEach((index) => merged.add(index));
        return Array.from(merged).sort((first, second) => first - second);
      });
    };

    if ("requestIdleCallback" in window) {
      const idleId = window.requestIdleCallback(preloadNext, { timeout: 1200 });
      return () => window.cancelIdleCallback(idleId);
    }

    const timeoutId = window.setTimeout(preloadNext, 800);
  ...
  return (
    ...
    >
      {cardOrder.map((originalIndex, displayIndex) => (
        ...
        >
          <img
            src={loadedIndices.includes(originalIndex) ? imageList[originalIndex] : undefined}
            alt={`${altPrefix}, кадр ${originalIndex + 1}`}
